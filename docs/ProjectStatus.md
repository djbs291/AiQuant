...
14. **The batch path blends multi-symbol tick files.** The symbol is parsed into `Tick` and then dropped: `TickToCandleResampler` has no symbol field, so `resample_csv_with_stats` — and therefore `run_scenario`, the HTTP scenario endpoints and the Python module — merges every symbol in a file into one candle series without a word. Found while building the streaming skeleton, which **refuses** such a file instead (naming both symbols, with `--symbol` to pick one). Fixing the batch path properly means a symbol dimension above `Tick`, which is the same prerequisite the per-symbol streaming workers need, so it is deliberately left to that work rather than patched twice. Until then, a multi-symbol CSV is only safe through `aiquant stream`.
